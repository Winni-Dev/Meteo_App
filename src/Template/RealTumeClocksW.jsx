...
// Nouvelle animation pour l'effet glace sur l'horloge
const iceGlow = keyframes`
  ...
  50% {
    box-shadow: 
      ...
      0 0 35px rgba(173, 216, 230, 0.2),
      inset 0 0 15px rgba(173, 216, 230, 0.2);
  }
  100% {
    box-shadow: 
      0 0 5px rgba(173, 216, 230, 0.3),
...
const AppContainer = styled.div`
  background: ${({ darkMode, blur }) => 
    darkMode 
      ? '#000000'  // Fond noir pur pour le dark mode
...
const ProfileImageWrapper = styled.div`
  ...
  &::before {
    ...
    animation: ${pulse} 3s ease infinite, ${({ darkMode }) => darkMode ? css`${neonPulse} 3s ease infinite` : 'none'};
    z-index: 0;
  }
  ...
  &::after {
    ...
    background: ${({ darkMode }) => 
      darkMode 
        ? 'rgba(0, 0, 0, 0.9)'  // Fond plus sombre pour contraster avec le noir
...
const ProfileImage = styled.img`
  ...
  position: relative;
  z-index: 2;
  animation: ${({ darkMode }) => darkMode ? css`${gentleGlow} 4s ease-in-out infinite` : 'none'};
`;
...
const ClockContainer = styled.div`
  ...
  align-items: center;
  justify-content: center;
  animation: ${({ darkMode }) => darkMode ? css`${iceGlow} 3s ease-in-out infinite` : 'none'};
...
const RealTimeClockW = ({ image, name = "Votre Nom", message = "Voici un message personnalisé que vous pouvez modifier en passant une prop 'message' au composant." }) => {
